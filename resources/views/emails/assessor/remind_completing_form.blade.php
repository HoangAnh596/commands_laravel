...
<div>Theo kế hoạch Checkpoint {{ \Carbon\Carbon::parse($campaign->start_date)->format('m/Y') }}
    từ bộ phận Nhân sự, chỉ còn <strong>{{ $remainingDay }} ngày</strong> để Cán bộ quản lý trực tiếp hoàn thành phiếu đánh giá nhân viên.</div>
<p>Anh/chị vui lòng hoàn thành việc chỉ định này theo đúng deadline trước {{ config('app.time_deadline') }} của ngày <strong>{{ \Carbon\Carbon::parse($campaign->deadline_assessor_complete)->format('d/m/Y') }}</strong></p>
<p>Link phiếu đánh giá: <a href="{{ Common::getAppUrl() . 'form-checkpoint/' . $assessor->checkpoint_id }}">{{ Common::getAppUrl() . 'form-checkpoint/' . $assessor->checkpoint_id }}</a></p>
